...
router.put('/:id', async (req, res) => {
  const { name, avatar_color, avatar_url, cuisines, dietary_restrictions, dislikes, likes } = req.body;
  const member = await FamilyMember.findByIdAndUpdate(
    req.params.id,
    { name, avatar_color, avatar_url, cuisines, dietary_restrictions, dislikes, likes },
